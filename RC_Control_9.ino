...
void loop() {
  ...
  int rc4INT;
  int RotTrim;
  int MaxPower = 55; //MaxPower provided by the lift fans
  int MaxTorque = 45; //Additional power that torque can provide (100 = MaxPower + MaxTorque
  float scale = 0;
  float RightPower;
  float LeftPower; 
  float ForwardPower;
  ...
  throttle = map(val, 1020, 1900, 0, MaxPower*10);
  ...
  if(rc4 > 1500) {//right stick
    ...
    RightPower = throttle + (throttle*MaxTorque/MaxPower)*(scale/100) + 1000;
    LeftPower  = throttle - (throttle*MaxTorque/MaxPower)*(scale/100) + 1000;
